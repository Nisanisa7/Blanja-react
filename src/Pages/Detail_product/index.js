...
import { addCart, getSelectedProduct, updateStock } from '../../configs/redux/actions/productsAction';
...
const Product =  () => {
    ...
    dispatch(getSelectedProduct(id, (data)=>{
        setDetail(data)
    }))
    }, [])
    ...
    const handleClick = () =>{
        if(count >= 1){
            settotalPerItem(detail.stock - count)
            const sendData = {
                stock: totalPerItem,
              };
              console.log(sendData);
            dispatch(updateStock(sendData, detail.idProduct))
        }
        ...
        <div>
            ...
            <ToastContainer />
             ...
             {/* <nav style="--bs{`breadcrumb ${style.breadCrumb-`}divider: '>';" aria-label={`breadcrumb ${style.breadCrumb"`}> */}
                    ...
                    <li className={`breadcrumb-item active`} aria-current="page">{detail.idCategory}</li>
                    </ol>
              </nav>
                ...
                <div className={style.wrapper_Content}>
                        ...
                        <div className={style.button_wrapper}>
                            ...
                            <div className={style.button_rigth}>
                                <Button_count plus={()=> handleIncrement()} minus={()=> handleDecrement()} counter={count}/>
                            </div>

                        </div>
                        <div className={style.button_content}>
                        <button type="button" className={`btn btn-outline-dark ${style.chat}`}>Chat</button>
                        <button type="button"  onClick={handleClick} className={`btn btn-outline-dark ${style.addbag}`}>Add bag</button>
                        <button type="button" onClick={handleBuyNow} className={`btn btn-danger ${style.buynow}`}>Buy Now</button>
                        </div>
                </div>
