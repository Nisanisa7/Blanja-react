...
function Profile_seller() {
    ...
    const formik = useFormik({
        ...
        onSubmit: values=>{
            ...
            console.log(form);
            axios.patch(`http://localhost:4000/v1/profile/${idSeller}`, form )
            .then((res) => {
            ...
            localStorage.setItem('store_description', dataResponse.store_description)
                ...
                <div className={style.parent_Flex}>
                    ...
                    <div className={style.inputProfile}>
                        <div className={style.inputP1}>
                            <input className={formik.errors.store_name ? 'errorborder_profile' : style.inpt} type="text" name="store_name"  value={formik.values.store_name} onChange={formik.handleChange} placeholder={store_name}/>
                        ...
                        <div className={style.inputP2}>
                            <input className={style.inpt} disabled type="text" placeholder={email}/>
                        </div>
                        <div className={style.inputP3}>
                            <input className={formik.errors.phone_number ? 'errorborder_profile' : style.inpt} type="text" name="phone_number" value={formik.values.phone_number}onChange={formik.handleChange} placeholder={phone_number}/>
                        ...
                        <div className={style.inputP4}>
                            <input className={formik.errors.store_description ? 'errorborder_profile' :  style.inpt} type="text" name="store_description" value={formik.values.store_description} onChange={formik.handleChange} placeholder={store_description}/>
                            {formik.errors.store_description && formik.touched.store_description && ( <p className='errors'>{formik.errors.store_description}</p>)}
                        </div>
                    </div>
                    <div className ={style.vertical}></div>
                    <div className={style.profilepic}>
                         <div className={style.imageRight}>
                            <img className={style.profile_picture} src={preview ? preview : image ? image : Profile} alt=""/>
                        </div>                
                        <div className={style.button_Wrap}>
                            <label className={style.button} for="upload">Save Change</label>
                        ...
                        {errImageSize ? <p className={style.error}>Image size is too large. <br /> max 1mb</p> : ''}
                        {errImageType ? <p className={style.error}>Invalid file type.  <br/> only png, jpg, and jpeg  <br/> format allowed</p> : ''}
                    </div>
                 </div>
